...
adverbs = expr:((adverb_phrase _?)+ / trans_adverb) {return _node("adverbs", expr);}
adverb_phrase = expr:(modifiers? adverbial (_? connective _? adverb_phrase)?) {return _node("adverb_phrase", expr);}
adverbial = expr:(adverb_compound / adverb / onomatopoeia) {return _node("adverbial", expr);}
...
GU_elidible = expr:(SS_terminator?) {return (expr === "" || !expr) ? ["GU"] : _node_empty("GU_elidible", expr);} /// &((_? adverbs _?)? (connective? _? modifiers? illocution / connective? _? modifiers? pronoun / connective? _? modifiers? predicate_term / connective _? modifiers? adverb_phrase / connective? _? modifiers? quoter / connective? _? modifiers? preposition / connective? _? modifiers? determiner / connective? _? modifiers? transmogrifier / connective _? (tag / binder) / LS_terminator / DS_terminator / ALL_terminator / end_of_input))
...
adverb_compound = expr:(adverb adverb_suffix) {return _node("adverb_compound", expr);}
adverb = expr:(!(root / freeword) (q e u / v o u / v a i / n o i / n a i / z e i / k e i / q u o / x o u / f o u / f i a / f i e / f i o / f i u / w a / w e / w i / w o / w u / n a !ANY_V / x a !ANY_V / f a !ANY_V / p a !ANY_V)) {return _node("adverb", expr);}
