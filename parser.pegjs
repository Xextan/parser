...
serial = expr:(modifiers? _? verb ( _? serial / _? verb)+) {return _node("serial", expr);}
...
free_term = expr:(_? modifiers? (pronoun / determiner_SS _? (term_nucleus / _? (SS_terminator / &(case_marker / preposition / transmogrifier / illocution / end_of_input))) / determiner_LS _? clause _? (LS_terminator / &(illocution / end_of_input))) (_? connective _? free_term)?) {return _node("free_term", expr);}
