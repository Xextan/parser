...
predicate = expr:((( _? modifiers? (serial / verbal) (_? (tag_phrase / binder_phrase))*) / _? !noun_term modifiers &(illocution / end_of_input)) tag_phrase? (_? connective _? predicate)?) {return _node("predicate", expr);}
serial = expr:(modifiers? _? verbal ( _? serial / _? modifiers? verbal)+) {return _node("serial", expr);}
verbal = expr:(verb _? (binder_phrase / connective _? predicate _? GU_elidible)*) {return _node("verbal", expr);}
